...
提供常规关闭内屏和**驱动合盖实验模式**，不改变铰链传感器读数。**M3 用户实测 v0.2、v0.3 均能列出第二外屏，但第二台仍无信号；v0.2 还出现过恢复内屏偏紫。当前 v0.3.1 是诊断版，尚未解决 M3 开盖双外屏输出。**
...
- v0.3 为新增外屏选择其实际输出通道，恢复时重新识别内屏并恢复原模式；画面确认独立于显示器枚举。用户随后报告 M3 的第二台仍无信号。仅凭该现象无法确定选择请求是否执行、是否被接受，以及链路停在哪一步。
...
驱动后端、v0.3 恢复顺序与验证边界见 [驱动合盖实现](docs/DIRECT-ROUTING.md)。
