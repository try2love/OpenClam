...
提供常规关闭内屏和**驱动合盖实验模式**，不改变铰链传感器读数。**M3 用户实测 v0.2 能列出第二外屏，但没有信号，恢复内屏还可能偏紫。v0.3 是针对这两个问题的研究预览版，尚未确认解决 M3 开盖双外屏。**
...
- v0.3 为新增外屏选择其实际输出通道，恢复时重新识别内屏并恢复原模式；画面确认独立于显示器枚举。M4 通道选择接口返回不支持，M3 使用不同的显示服务，其支持情况仍待实测。
