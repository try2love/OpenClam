...
提供常规关闭内屏和**驱动合盖实验模式**，不改变铰链传感器读数。**M3 用户实测至 v0.3.1，第二台外屏仍无信号；v0.2 还出现过恢复内屏偏紫。v0.3.2 调整实验模式的内屏关闭步骤，移除额外的面板关闭请求，尚未确认解决 M3 开盖双外屏输出。**
...
- **启用双外屏模式（实验）**：尝试关闭内屏并切换输出通道。请检查两台外屏都有正常画面后再点击保留；30 秒未确认会自动恢复内屏。系统列出两台显示器不代表它们已收到信号。
...
M3 可通过“启用双外屏模式（实验）”尝试，保持两台外屏线缆连接；开盖时只识别一台外屏也允许开始。若第二外屏无信号，请选择恢复内屏。接口不支持、通道无法确认或显示状态未稳定时会自动恢复。若恢复后颜色异常，请退出 OpenClam 后合盖再开盖；该异常仍需要 M3 实机验证。
...
- v0.3.2 仅在双外屏实验模式中移除额外的面板关闭请求，保留布局关闭、输出切换、画面确认和恢复机制；常规关闭内屏模式不变。这项调整用于验证额外电源请求是否妨碍共享输出恢复，目前尚无 M3 成功结果。本轮已在 M4 连接两台外屏时验证 30 秒未确认自动恢复，以及切换中、预览中强杀主程序、辅助程序运行中强杀守护后的恢复；内屏原模式核对通过，真实像素和色彩仍需目视验收。
- 不承诺改善温度或性能；尚无温度对照实验。
...
驱动后端、M3 现场发现、v0.3.2 实验调整与验证边界见 [驱动合盖实现](docs/DIRECT-ROUTING.md)。
